...
module Funkytown
  ...
  # Load options from host-specific yml file and set as instance vars
    ...
    def start_servant
      servant_hosts.each do |host|
        run_servant(host) if is_localhost?(host)
      end
    end

    def run_servant(servant_host = "localhost")
    ...
    def stop_servant
      servant = servant_process("localhost")
      if !servant.is_running?
...
#    end
    ...
    def check_servant(servant_host)
      process = servant_process(servant_host)
      if !process.is_running?
        if is_localhost?(servant_host)
          run_servant(servant_host)
